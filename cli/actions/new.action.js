...
function generateApplicationFiles(options) {
    defaultValue.paths.forEach(value => {
        ...
        createOrUpdateFile(source, destination, { isOnlyCopy: path[0].isOnlyCopy, moduleName: path[0].isOnlyCopy ? options.name : null, databaseType: options.database.toLowerCase()})
    });
}
